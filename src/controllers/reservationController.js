...
const createReservation = async (req, res) => {
  try {
    ...
    for (const reservation of existingReservations) {
      const reservationArrivee = parse(
        reservation.dateArrivee,
        "dd/MM/yyyy",
        new Date()
      );
      const reservationDepart = parse(
      ...
      if (
        (formattedDateArrivee >= reservationArrivee &&
          formattedDateArrivee < reservationDepart) ||
        (formattedDateDepart > reservationArrivee &&
          formattedDateDepart <= reservationDepart) ||
        (formattedDateArrivee <= reservationArrivee &&
          formattedDateDepart >= reservationDepart)
      ) {
        return res
          .status(409)
          .json({ message: "Il y a déjà une réservation pour ces dates" });
      }
    }

    if (
      isNaN(formattedDateArrivee.getTime()) ||
      isNaN(formattedDateDepart.getTime())
    ) {
      return res
        .status(400)
    ...
    const dureeLocation = differenceInDays(
      formattedDateDepart,
...
const getReservationByDates = async (req, res) => {
  try {
    ...
    if (!reservation) {
      return res
        .status(404)
        .json({ message: "Réservation introuvable pour ces dates" });
    }
    res.json({ reservation });
  } catch (error) {
    res.status(500).json({
...
const getReservationByDay = async (req, res) => {
  try {
    ...
    if (!reservation) {
      return res
        .status(200)
        .json({ message: "Aucune réservation pour cette journée" });
    }
    res.json({ reservation });
  } catch (error) {
    res.status(500).json({
...
const updateReservation = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Réservation modifiée avec succès :", reservation });
  } catch (error) {
    res.status(500).json({
